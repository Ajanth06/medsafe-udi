...
import { supabase } from "../../lib/supabaseClient"; // WICHTIG: zwei Punkte!
...
export default function AuthBar() {
  ...
  // Aktuellen User laden & auf Änderungen hören
  useEffect(() => {
    let ignore = false;
  ...
  const handleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email: email.trim(),
        options: {
          emailRedirectTo: window.location.origin, // zurück zu deiner App
        },
      });
